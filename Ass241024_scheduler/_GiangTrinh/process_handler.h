...
********************************************************************************/
    ...
    #define INIT_PROC(pid_ptr) /*parameter is a pointer* CHILD_PID */\
    {\  
        STARTUPINFO si;\
        char path_param[256];\
        ...
        sprintf(path_param,"\"%s\" child",argv[0]);\
        \
        if (!CreateProcess(\
    ...
    #define INIT_PROC(pid_ptr) /* return -1 if create process failed, 0 if success */\
    {\
        *pid_ptr = fork();\
        if(*pid_ptr == -1)\
        {return -1;}\
